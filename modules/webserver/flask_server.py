...
class FlaskServer:
    ...
    def __init__(self, name="RaspberryPi Server", host="0.0.0.0", port=5000, debug=False):
        """
    ...
    def _setup_default_routes(self):
        ...
        def control_panel():
            """Control panel page."""
            return render_template_string(self._get_control_template(),
                                        server_name=self.name,
                                        data_keys=list(self.shared_data.keys()))
...
<body>
    ...
    def stop(self):
        ...
        if self.server_thread and self.server_thread.is_alive():
            self.is_running = False
            print("Server stop requested (note: Flask dev server cannot be stopped gracefully)")
    ...
    def get_url(self, endpoint='index'):
        ...
        Returns:
            str: Full URL
        """
        return f"http://{self.host}:{self.port}" + (f"/{endpoint}" if endpoint != 'index' else "")
